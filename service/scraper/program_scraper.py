...
class ProgramScraper:
    ...
    def _fetch_all_programs():
        ...
        while True:
            ...
            logger.info(f"Fetching programs page {page}...")
...
if __name__ == "__main__":
    ...
    all_handles = scraper.all_handles_scraping()
    high = scraper.high_priority_handle_scraping()
    low = scraper.low_priority_handle_scraping()
    
    print(f"Total programs: {len(all_handles)}")
    print(f"High priority: {len(high)}")
    print(f"Low priority: {len(low)}")
    
    # Check Freshworks
    fw = scraper.find_specific_program("freshworks")
    print(f"\nFreshworks in HIGH: {'freshworks' in [h.lower() for h in high]}")
    print(f"Freshworks in LOW: {'freshworks' in [h.lower() for h in low]}")