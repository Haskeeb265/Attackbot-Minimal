...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
AssetfinderTool — passive subdomain enumeration via tomnomnom's assetfinder.
...
class AssetfinderTool(ReconTool):
    """Passive subdomain discovery using assetfinder."""

    name    = "AssetfinderTool"
    binary  = "assetfinder"
    timeout = 300
    ...
    async def run(
        ...
        for target in targets:
            cmd = [self.binary, "--subs-only", target]
            returncode, stdout, stderr = await self._run_subprocess(cmd)

            if returncode != 0:
                log.warning("assetfinder failed for %s: %s", target, stderr.strip())
                results.append(self._make_result(
                    target, data={}, raw_output=stderr,
                    success=False, error=f"assetfinder exited {returncode}",
                ))
                continue
            ...
            log.info("assetfinder: %s -> %d subdomains", target, len(subdomains))
            results.append(self._make_result(
                target,
